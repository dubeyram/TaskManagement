...
# Signal when any new user is created
...
def user_task_assigned_(signal, task, extra_data, **kwargs):
    print(f"Task: {task.get('name')} is assigned to a these users: {extra_data.get('user_id')}")
...
def task_created(signal, task, extra_data, **kwargs):
    if extra_data.get('created'):
        if not task.get('assigned_users'):
            print(f"New task: {task.get('name')} created successfully, but not assigned to any users yet.")
        else:
            print(f"New task: {task.get('name')} created successfully, and assigned to these users: {task.get('assigned_users')}")
    else:
